  ...
  return (
    <section className="bg-white w-full shadow-lg h-full p-6 flex flex-col mx-auto hover:shadow-xl transition-shadow duration-300">
      <header className="mb-6 text-center">
        <h2 className="text-2xl font-semibold text-blue-500">Readers' Reviews</h2>
      ...
      <div className="divide-y divide-gray-200 w-full max-h-96 overflow-y-auto scrollbar-hide">
        {reviews.length === 0 && !reviewsLoading ? (
          ...
          reviews.map((r) => (
            <article key={r.id} className="py-4 relative group hover:bg-gray-300 transition-colors duration-200">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {editingReviewId === r.id ? (
                    <form onSubmit={handleSubmitEdit(onEditSubmit)} className="mt-4 w-full space-y-4">
                      <textarea
                      ...
                      />
                      {editErrors.comment && <p className="text-sm text-red-600 mt-1">{editErrors.comment.message}</p>}

                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <StarRating rating={editRating} setRating={(val) => setEditValue("rating", val)} />
                        <div className="flex items-center gap-3">
                          <button type="button" onClick={cancelEditing} className="px-5 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">
                            Cancel
                          </button>
                          <button type="submit" disabled={editSubmitting} className="px-5 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50">
                            {editSubmitting ? "Saving..." : "Save changes"}
                          </button>
                        </div>
                      </div>
                      {editError && <p className="text-sm text-red-500 mt-2 text-center">{editError}</p>}
                    </form>
                  ) : (
                    <p className="mt-2 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{r.comment}</p>
                  )}
                </div>
              </div>
            </article>
